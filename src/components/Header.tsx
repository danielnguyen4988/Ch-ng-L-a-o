...
export const Header: React.FC<HeaderProps> = ({
  ...
  const { isConnected: isDriveConnected, files: driveFiles } = useGoogleDrive();
        ...
        <div className="flex items-center gap-2 order-2 sm:order-3 ml-auto sm:ml-0">
          ...
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-bold transition-all cursor-pointer border shadow-sm ${
              isDriveConnected
                ? 'bg-teal-950/50 hover:bg-teal-900/60 text-teal-300 border-teal-500/40'
                : 'bg-slate-900 hover:bg-slate-800 text-slate-300 border-slate-700'
            }`}
            title="Kho Lưu Trữ Bằng Chứng & Hồ Sơ Trên Google Drive"
          >
            <Cloud className={`w-3.5 h-3.5 ${isDriveConnected ? 'text-teal-400' : 'text-slate-400'}`} />
            <span className="hidden lg:inline">Google Drive</span>
            {isDriveConnected && (
              <span className="w-1.5 h-1.5 rounded-full bg-teal-400 animate-pulse hidden sm:inline"></span>
            )}
          </button>
                ...
                <div className="absolute right-0 mt-2 w-72 bg-slate-900 border border-slate-700 rounded-2xl shadow-2xl overflow-hidden z-50 animate-in fade-in zoom-in-95 duration-150">
                  ...
                  <div className="p-1.5 space-y-0.5 text-xs">
                    <button
                      ...
                      className="w-full text-left px-3 py-2 rounded-xl text-slate-200 hover:bg-slate-800 hover:text-white flex items-center justify-between gap-2 cursor-pointer transition-all"
                    >
                      <div className="flex items-center gap-2">
                        <Cloud className="w-3.5 h-3.5 text-sky-400" />
                        <span>Kho Bằng Chứng Google Drive</span>
                      </div>
                      <span className={`text-[10px] px-1.5 py-0.2 rounded font-mono ${isDriveConnected ? 'bg-teal-500/20 text-teal-300' : 'bg-slate-800 text-slate-500'}`}>
                        {isDriveConnected ? `${driveFiles.length} tệp` : 'Chưa nối'}
                      </span>
                    </button>
