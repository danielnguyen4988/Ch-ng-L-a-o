...
import { Scale, XCircle, Copy, FileText, CheckCircle2, Cloud, ExternalLink, Loader2 } from 'lucide-react';
...
export const LegalDossierModal: React.FC<LegalDossierModalProps> = ({
  ...
  const { isConnected, isSyncing, saveDossier, signInWithDrive } = useGoogleDrive();
...
Người làm đơn: (Ký và ghi rõ họ tên)`;
  ...
  const copyLegalForm = () => {
    const text = generateFullLegalText();
  ...
  const handleSaveToDrive = async () => {
    try {
      ...
      if (!isConnected) {
        ...
        if (!ok) {
          onNotify('Chưa đăng nhập Google Drive.');
          return;
        }
      }
      const text = generateFullLegalText();
      const fileName = `VeraFense_DonToGiac_A05_${new Date().toISOString().slice(0, 10)}_${Date.now().toString().slice(-4)}.txt`;
      const res = await saveDossier(fileName, text);
      if (res) {
        setSavedDriveLink(res.webViewLink || null);
        onNotify('Đã lưu hồ sơ tố giác vào Google Drive thành công!');
      }
    } catch (err: any) {
      onNotify('Lưu hồ sơ lên Google Drive thất bại: ' + (err?.message || ''));
    } finally {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/85 backdrop-blur-md flex items-center justify-center p-3 sm:p-4 overflow-y-auto">
      <div className="bg-slate-950 border border-amber-500/60 w-full max-w-3xl rounded-3xl p-6 sm:p-8 shadow-2xl space-y-6 my-auto max-h-[92vh] overflow-y-auto font-sans">
        ...
        {savedDriveLink && (
          <div className="p-3 bg-teal-950/50 border border-teal-500/40 rounded-xl flex items-center justify-between gap-3 text-xs">
            <div className="flex items-center gap-2 text-teal-300">
              <CheckCircle2 className="w-4 h-4 text-teal-400 shrink-0" />
              <span>Đã lưu hồ sơ vào thư mục <strong>VeraFense_HoSo_ToGiac</strong> trên Google Drive của bạn!</span>
            </div>
            <a
              href={savedDriveLink}
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center gap-1.5 px-3 py-1 bg-teal-500 hover:bg-teal-400 text-slate-950 font-bold rounded-lg shrink-0 transition-colors shadow-sm"
            >
        ...
        <div className="flex flex-col sm:flex-row gap-2.5">
          ...
          <button
            ...
            disabled={isSavingToDrive || isSyncing}
            className="bg-teal-700 hover:bg-teal-600 active:bg-teal-800 text-white font-bold py-3 px-4 rounded-xl text-xs flex items-center justify-center gap-2 cursor-pointer transition-all shadow-lg shadow-teal-700/20 disabled:opacity-50"
            title="Lưu trữ đơn tố giác và bằng chứng vào Google Drive"
          >
            {isSavingToDrive ? (
            ...
            <span>{isSavingToDrive ? 'Đang Lưu Drive...' : 'Lưu Lên Google Drive'}</span>
          </button>
          <button
            onClick={() => window.print()}
            className="bg-slate-800 hover:bg-slate-700 text-slate-200 font-bold py-3 px-4 rounded-xl text-xs flex items-center justify-center gap-2 cursor-pointer transition-colors"
          >
