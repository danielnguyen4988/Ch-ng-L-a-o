...
export const AuthModal: React.FC<AuthModalProps> = ({
  ...
  const { register, login, updateUser } = useAccount();
  const { signInWithDrive, isConnected } = useGoogleDrive();
  ...
  const handleGoogleSignIn = async () => {
    try {
      ...
      if (success) {
        onNotify('Đăng nhập Google thành công! Đã kích hoạt lưu trữ Google Drive.');
        onClose();
      } else {
        onNotify('Đăng nhập Google không thành công hoặc người dùng đã hủy.');
      }
    } catch {
      onNotify('Lỗi trong quá trình đăng nhập Google.');
    } finally {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-slate-950/80 backdrop-blur-md animate-in fade-in duration-200">
      <div className="bg-slate-900 border border-slate-700 w-full max-w-md rounded-2xl shadow-2xl overflow-hidden flex flex-col">
        ...
        <form onSubmit={handleSubmit} className="p-4 sm:p-5 space-y-3.5 text-xs sm:text-sm">
          ...
          {/* GOOGLE DRIVE SIGN-IN DIVIDER */}
          <div className="relative flex items-center justify-center my-1">
            <div className="border-t border-slate-800 w-full"></div>
            <span className="bg-slate-900 px-3 text-[11px] text-slate-500 uppercase tracking-wider whitespace-nowrap">
              Hoặc tiếp tục với
            </span>
            <div className="border-t border-slate-800 w-full"></div>
          </div>
          ...
          <div className="flex justify-center">
            <GoogleSignInButton
              ...
              text={isGoogleLoading ? 'Đang kết nối Google...' : 'Sign in with Google (Đồng bộ Drive)'}
